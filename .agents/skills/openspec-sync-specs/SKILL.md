...
4. **For each delta spec, apply changes to main specs**
   ...
   d. **Create new main spec** if capability doesn't exist yet:
      - If the delta contains any MODIFIED or RENAMED requirement, report the missing target and stop, even when the same delta also contains ADDED requirements
      - Otherwise, if the delta contains ADDED requirements:
