...
impl Plugin<TypescriptLanguage> for TsProtoPlugin {
    ...
    fn activate(&self, repository: &RepositorySnapshot) -> Option<PluginActivation> {
        repository
            ...
            .iter()
            .filter(|input| {
                SourceLanguage::from_path(&input.path)
                    .is_some_and(|language| language != SourceLanguage::Svelte)
            })
            .filter_map(|input| {
...
impl SourceRecognizer<TypescriptLanguage> for TsProtoPlugin {
    fn recognize(
        ...
        analysis.generated = analysis.language != SourceLanguage::Svelte
            && ts_proto::is_generated_source(input.path, input.text);
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn ts_proto_plugin_ignores_svelte_sources() {
        let repository = snapshot(&[(
            "src/client.generated.svelte",
            "<script>// generated by ts-proto</script>",
        )]);
        ...
        assert!(
            active
                .plugins()
                .all(|plugin| plugin.metadata.id != "typescript.ts-proto")
        );
    }
