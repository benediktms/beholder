Warning: truncated output (original token count: 41456)
Total output lines: 4258
...
fn index_workspace_through_port(
    ...
    tracing::info!(
        ...
        inventory_membership_or_metadata_hashes = inventory_statistics.membe…11456 tokens truncated…_FRONTEND_VERSION}:{ELIXIR_RESOLVER_VERSION}:typescript:{TYPESCRIPT_FRONTEND_VERSION}:{TYPESCRIPT_RESOLVER_VERSION}:graphql:{GRAPHQL_FRONTEND_VERSION}"
            )
        );
